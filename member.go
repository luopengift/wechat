...
//删除成员
func (self *MemberCtx) Delete(id int) {
...
//获取成员
func (self *MemberCtx) GetUser(userid string) {
	url := fmt.Sprintf("https://qyapi.weixin.qq.com/cgi-bin/user/get?access_token=%s&userid=%d", self.GetToken(), userid)
	resp, err := http.Get(url)
	if err != nil {
		logger.Error("",err)
		return
...
*/
func (self *MemberCtx) GetSimpleUserListByDepartment(department_id int) {
...
//获取部门成员(详情)
func (self *MemberCtx) GetUserListByDepartment(department_id int) {
